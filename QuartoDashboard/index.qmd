...
geoDataFiltered <- reactive({
  df.geo_budget %>%
    filter(Country %in% input$countrySelect)
})

geoDataRenderable <- reactive({
  data.frame(City = geoDataFiltered()$City,
             Budget = geoDataFiltered()$Budget,
             lon = geoDataFiltered()$lon,
             lat = geoDataFiltered()$lat)
})
...
p <- ggplot() +
  geom_polygon(
    ...
    fill = "#DEDEDE",
    colour = "#818181",
    size = .15 #TODO: linewidth
  ) +
  geom_point(
    data = df.geo_budget_react(),
    aes(x = lon, y = lat, name = City, size = Budget), #TODO: paste City, Country
    color = "red",
    alpha = .6
  ) +
  coord_cartesian(xlim = c(-30,50), ylim = c(32,70)) +
