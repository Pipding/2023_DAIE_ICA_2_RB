...
checkboxGroupInput(
  ...
  selected = sort(unique(df.geo_budget$Country))[1], # For demo purposes, only select the first entry
  inline = FALSE,
  width = '100%'
)
...
df.timeline_data_react <- reactive({
  df.timeline_data %>%
    filter(start >= input$projectStartRange[1]) %>%
    filter(start <= input$projectStartRange[2]) %>%
    filter(end >= input$projectEndRange[1]) %>%
    filter(end <= input$projectEndRange[2])
})
